...
工作空间（项目级组织单元）
  ├── 人员管理（角色权限体系）
  ├── 对话 Session（= 一个需求，名称即需求名称）
  │   └── Session 完成后自动生成 Wiki 页面
...
一条名为"采购订单审批流程优化"的 Session（需求），背后需要补充：
- 当前审批流程的详细描述（流程图、节点角色）
- 与 SAP 接口的对接方式
- 业务方提出的特殊约束条件
- 与"采购订单自动生成"需求的关联关系

这些内容不适合在对话 Session 中反复讨论，而是需要一个**独立的 Wiki 页面**来沉淀，并且可以在页面间建立引用关系。

### 3.2 Wiki 页面的生命周期

```
一个 Session（需求名称）完成挖需 → 达到完整度阈值
       ↓
LLM 自动从 Session 对话历史中提取内容，生成 Wiki 页面（初始版本）
       ↓
人工修改 / 补充 Wiki 内容
       ↓
在 Wiki 中添加 [[链接]] 引用其他需求（其他 Session 的 Wiki 页面）
       ↓
图谱自动更新 → 需求关系可视化
```
...
| # | 需求 | 验收标准 |
|---|------|----------|
|| 7.1 | **自动创建 Wiki 页面** | Session 名称即为需求名称。当 Session 达到完整度阈值（如 0.75）并生成了 PRD 后，自动在该工作空间下创建一个 Wiki 页面，标题 = Session 名称（即需求名称），LLM 从 Session 对话历史中提取内容填充各章节 |
| 7.2 | **查看 Wiki 页面** | 每个 Session 旁显示「📄 查看 Wiki」链接，点击打开该 Session（需求）对应的 Wiki 页面 |
| 7.3 | **编辑 Wiki 页面** | 支持 Markdown 编辑器编辑 Wiki 内容，实时保存（或手动保存按钮），语法高亮 + 预览双栏模式 |
| 7.4 | **Wiki 页面列表** | 工作空间内提供「Wiki 文库」入口，以列表展示所有 Wiki 页面（标题 = 需求名称、创建时间、最后编辑者、所属 Session） |
...
| 8.2 | **手动关联 Session** | 空白 Wiki 页面可关联到工作空间中的某个 Session（下拉选择），也可不关联（作为通用知识页面） |
| 8.3 | **知识沉淀** | Wiki 页面支持不关联 Session，作为工作空间的通用知识库页面（如项目约定、API 文档、架构决策记录 ADR） |
...
| 9.2 | **AI 提炼对话** | 当 Wiki 页面关联某个 Session 时，AI 可提取该 Session 中的关键对话片段（决策依据、技术选型、业务方意见），追加到 Wiki 的"决策记录"章节 |
...
PM 在一个 Session（名称=需求名称）中完成挖需对话后，不希望手动为这个需求写 Wiki 页面。AI 应该自动完成：
...
| # | 需求 | 验收标准 |
|---|------|----------|
|| 16.1 | **Session 完成时自动创建** | 当 Session 达到完整度阈值并生成了 PRD 后，自动创建该需求的 Wiki 页面，标题 = Session 名称（即需求名称），LLM 从整个 Session 对话历史中提取内容填充各章节 |
| 16.2 | **实时生成（配置项）** | 用户可选：是否在 Session 达到阈值时立即生成 Wiki，还是等 PRD 生成完成后再批量创建（默认：PRD 完成后） |
| 16.3 | **AI 检测关联** | 生成 Wiki 时，AI 自动检测该需求是否与工作空间内已有 Wiki 页面（其他 Session 的 Wiki）相关，自动插入 `[[引用]]` |
...
| # | 需求 | 验收标准 |
|---|------|----------|
| 17.1 | **AI 补充内容** | 在已有 Wiki 页面中点「AI 补充」，AI 根据该 Session 中的对话内容，更新 Wiki 的对应章节。如果该需求后续有新的讨论 Session，也可选择参考 |
...
CREATE TABLE wiki_pages (
    ...
    session_id      INT DEFAULT NULL,             -- 关联的 Session ID（Session = 需求，可为 NULL 用于纯知识页面）
    title           VARCHAR(255) NOT NULL,        -- 页面标题 = 需求名称
...
| 现有模块 | 影响 |
|---------|------|
| 01-对话挖需引擎 | Session 必须在工作空间下创建；Session 名称 = 需求名称；Session 结束后自动生成 Wiki 页面 |
| 02-PRD生成管线 | PRD 生成时引用关联的 Wiki 内容作为背景知识 |
| 03-用户与权限 | 新增工作空间级角色，与全局角色叠加生效 |
| 04-存储与数据 | 新增 6 张表，修改 sessions 表（增加 workspace_id） |
| 06-前端界面 | 重构入口页（工作空间列表取代扁平导航），新增 Wiki 编辑器、图谱页面、成员管理页面 |
| 07-导入与知识库 | 导入的文档内容可以关联到 Session（需求），或直接写入关联 Session 的 Wiki 页面 |
