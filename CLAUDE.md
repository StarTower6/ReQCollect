...
- **MUST 先运行 `npx gitnexus analyze`** 确保索引最新
- **MUST 使用 `gitnexus_query` 或 `mcp__gitnexus__query` 查找相关符号和 execution flow**
...
This project is indexed by GitNexus as **ReQCollect** (1848 symbols, 3407 relationships, 112 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.

> If any GitNexus tool warns the index is stale, run `npx gitnexus analyze` in terminal first.
>
> 索引自动更新: post-commit hook 在 `git push` 成功后自动运行 `npx gitnexus analyze`，无需手动操作。
